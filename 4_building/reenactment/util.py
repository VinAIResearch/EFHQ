...
def crop_bbox_from_frames_custom(
    frame_list,
    tube_bbox,
    image_shape=(256, 256),
    min_size=200,
    ...
    # Filter if it is too small
    if max(width, height) < min_size:
        print("too small", max(width, height))
        return None, None
    ...
    if (
        ...
        print("Too near border")
        return [None, None]

    selected = [
        frame[max(0, top) : min(frame_shape[0], bot), max(0, left) : min(frame_shape[1], right)]
...
def scheduler_vfhq(data_list, fn, args):
    device_ids = args.device_ids.split(",")
    pool = Pool(processes=args.workers)
    args_list = cycle([args])
    for chunks_data in tqdm(pool.imap_unordered(fn, zip(data_list, cycle(device_ids), args_list))):
...
def save(path, frames, format):
    if format == ".mp4":
        ...
        for j, frame in enumerate(frames):
            if not os.path.exists(os.path.join(path, str(j).zfill(7) + ".png")):
