...
def download_files(file_specs, num_threads=32, status_delay=0.2, timing_window=50, **download_kwargs):
    ...
    for spec in missing_specs:
        spec_queue.put(spec)
    thread_kwargs = dict(
        spec_queue=spec_queue, exception_queue=exception_queue, stats=stats, download_kwargs=download_kwargs
    )
...
def recreate_aligned_images_fast(
    json_data, dst_dir="realign1024x1024", output_size=1024, transform_size=4096, enable_padding=True, start_index=0
):
...
def run_cmdline(argv):
    ...
    parser.add_argument(
        "-j", "--json", help="download metadata as JSON (254 MB)", dest="tasks", action="append_const", const="json"
    )
    parser.add_argument(
        "-s", "--stats", help="print statistics about the dataset", dest="tasks", action="append_const", const="stats"
    )
    parser.add_argument(
        "-i",
        "--images",
    ...
    parser.add_argument(
        "--chunk_size", help="chunk size for each download thread (default: 128)", type=int, default=128, metavar="KB"
    )
    parser.add_argument(
        "--num_attempts",
    ...
    parser.add_argument(
        "--start_index", help="start index for alignment (default: 0)", type=int, default=0, metavar="NUM"
    )
