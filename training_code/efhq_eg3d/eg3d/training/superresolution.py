...
from training.networks_stylegan2 import Conv2dLayer, SynthesisBlock, SynthesisLayer, ToRGBLayer
...
class SuperresolutionHybridDeepfp32(torch.nn.Module):
    ...
    def forward(self, rgb, x, ws, **block_kwargs):
        ...
        if x.shape[-1] < self.input_resolution:
            x = torch.nn.functional.interpolate(
                x, size=(self.input_resolution, self.input_resolution), mode="bilinear", align_corners=False
            )
            rgb = torch.nn.functional.interpolate(
                rgb, size=(self.input_resolution, self.input_resolution), mode="bilinear", align_corners=False
            )
