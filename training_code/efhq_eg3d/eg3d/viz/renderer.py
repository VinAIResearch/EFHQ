...
class Renderer:
    ...
    def _tweak_network(self, net):
        ...
        if RELOAD_MODULES:
            ...
            net_new = (
                TriPlaneGenerator(*net.init_args, **net.init_kwargs).eval().requires_grad_(False).to(self._device)
            )
    ...
    def _render_impl(
        ...
        forward_cam2world_pose = LookAtPoseSampler.sample(
            3.14 / 2 + conditioning_yaw, 3.14 / 2 + conditioning_pitch, camera_pivot, radius=camera_radius
        )
        ...
        synthesis_kwargs = dnnlib.EasyDict(
            noise_mode=noise_mode, force_fp32=force_fp32, cache_backbone=do_backbone_caching
        )
