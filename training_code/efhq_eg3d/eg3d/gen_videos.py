...
def gen_interp_video(
    ...
    **video_kwargs,
    ...
    cam2world_pose = LookAtPoseSampler.sample(
        3.14 / 2, 3.14 / 2, camera_lookat_point, radius=G.rendering_kwargs["avg_camera_radius"], device=device
    )
            ...
            for xi in range(grid_w):
                ...
                if gen_shapes:
                    ...
                    samples, voxel_origin, voxel_size = create_samples(
                        N=voxel_resolution, voxel_origin=[0, 0, 0], cube_length=G.rendering_kwargs["box_warp"]
                    )
                    ...
                    if output_ply:
                        ...
                        convert_sdf_samples_to_ply(
                            ...
                            [0, 0, 0],
                            1,
                            os.path.join(outdir, f"{frame_idx:04d}_shape.ply"),
                            level=10,
                        )
                    else:  # output mrc
                        with mrcfile.new_mmap(
                            outdir + f"{frame_idx:04d}_shape.mrc", overwrite=True, shape=sigmas.shape, mrc_mode=2
                        ) as mrc:
                            mrc.data[:] = sigmas
