...
for subdir in os.listdir(realign1500_dir):
    if not os.path.isdir(os.path.join(realign1500_dir, subdir)):
        continue
    if not (len(subdir) == 5 and subdir.isnumeric()):
        continue
...
gdown.download(
    "https://drive.google.com/uc?id=1KdVf2lIepGECRaANGhfuR7mDpJ5nfb9K", "realign1500/cropping_params.json", quiet=False
)
...
gdown.download(
    "https://drive.google.com/uc?id=14mzYD1DxUjh7BGgeWKgXtLHWwvr-he1Z", "final_crops/dataset.json", quiet=False
)
