...
if __name__ == "__main__":
    ...
    for im_path, cropping_dict in tqdm(cropping_params.items()):
        ...
        _, im_high, _, _, = align_img(
            im,
            lm,
            np.array(cropping_dict["lm3d_std"]),
            target_size=1024.0,
            rescale_factor=cropping_dict["rescale_factor"],
        )
        ...
        im_cropped = im_cropped.resize(
            (cropping_dict["output_size"], cropping_dict["output_size"]), resample=Image.LANCZOS
        )
