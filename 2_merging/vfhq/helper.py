...
def parsevfhqtxt(txtfile, outputpath):
    NCOLS = 16
    with open(txtfile.as_posix()) as f:
        lines = f.readlines()
    lines = lines[7:-1]
    lines = list(map(lambda x: x.strip(), lines))
    res = {}
    for line in lines:
        numbers = list(map(lambda x: round(float(x), 2), line.split(r"\s+")))
...
def parseline_vfhq(line):  # Done
    NCOLS = 16
    numbers = line.split()
    numbers[2:] = list(map(lambda x: round(float(x), 2), numbers[2:]))
    frameid, idx, x1, y1, w, h = numbers[:6]
    x2, y2 = round(float(x1) + float(w), 2), round(float(y1) + float(h), 2)
    if len(numbers) == NCOLS:
        lmks = numbers[6:]
    else:
        lmks = None
    d = dict(frameid=frameid, x1=x1, y1=y1, x2=x2, y2=y2, lmks5pts=lmks)
    return d
