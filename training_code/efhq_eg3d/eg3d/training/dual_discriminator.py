...
from training.networks_stylegan2 import DiscriminatorBlock, DiscriminatorEpilogue, MappingNetwork
...
def filtered_resizing(image_orig_tensor, size, f, filter_mode="antialiased"):
    if filter_mode == "antialiased":
        ada_filtered_64 = torch.nn.functional.interpolate(
            image_orig_tensor, size=(size, size), mode="bilinear", align_corners=False, antialias=True
        )
    elif filter_mode == "classic":
        ada_filtered_64 = upfirdn2d.upsample2d(image_orig_tensor, f, up=2)
        ada_filtered_64 = torch.nn.functional.interpolate(
            ada_filtered_64, size=(size * 2 + 2, size * 2 + 2), mode="bilinear", align_corners=False
        )
        ...
        aliased = torch.nn.functional.interpolate(
            image_orig_tensor, size=(size, size), mode="bilinear", align_corners=False, antialias=False
        )
...
class DummyDualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        image_raw = (
            filtered_resizing(img["image_raw"], size=img["image"].shape[-1], f=self.resample_filter) * self.raw_fade
        )
        img = torch.cat([img["image"], image_raw], 1)
