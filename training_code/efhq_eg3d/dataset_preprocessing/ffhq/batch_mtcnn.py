    ...
    if not os.path.exists(dst):
        ...
        if len(result) > 0:
            ...
            if result[index]["confidence"] > 0.9:
                ...
                outLand.write(
                    str(float(keypoints["right_eye"][0])) + " " + str(float(keypoints["right_eye"][1])) + "\n"
                )
                outLand.write(str(float(keypoints["nose"][0])) + " " + str(float(keypoints["nose"][1])) + "\n")
                outLand.write(
                    str(float(keypoints["mouth_left"][0])) + " " + str(float(keypoints["mouth_left"][1])) + "\n"
                )
                outLand.write(
                    str(float(keypoints["mouth_right"][0])) + " " + str(float(keypoints["mouth_right"][1])) + "\n"
                )
                outLand.close()
                print(result)
