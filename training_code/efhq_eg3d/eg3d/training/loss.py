    ...
    def run_G(self, z, c, swapping_prob, neural_rendering_resolution, update_emas=False):
        if swapping_prob is not None:
            ...
            c_gen_conditioning = torch.where(
                torch.rand((c.shape[0], 1), device=c.device) < swapping_prob, c_swapped, c
            )
        else:
                ...
                torch.cat(
                    [
                        ...
                        torch.nn.functional.interpolate(
                            img["image_raw"], size=img["image"].shape[2:], mode="bilinear", antialias=True
                        ),
                    ],
                    dim=1,
                )
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        if (
            ...
            perturbed_coordinates = (
                initial_coordinates
                + torch.randn_like(initial_coordinates) * self.G.rendering_kwargs["density_reg_p_dist"]
            )
            ...
            TVloss = (
                torch.nn.functional.l1_loss(sigma_initial, sigma_perturbed) * self.G.rendering_kwargs["density_reg"]
            )
        ...
        if (
            ...
            perturbed_coordinates = (
                initial_coordinates
                + torch.tensor([0, 0, -1], device=ws.device) * (1 / 256) * self.G.rendering_kwargs["box_warp"]
            )  # Behind
            all_coordinates = torch.cat([initial_coordinates, perturbed_coordinates], dim=1)
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, update_emas=False)[
                "sigma"
            ...
            perturbed_coordinates = (
                initial_coordinates
                + torch.randn_like(initial_coordinates) * (1 / 256) * self.G.rendering_kwargs["box_warp"]
            )
            all_coordinates = torch.cat([initial_coordinates, perturbed_coordinates], dim=1)
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, update_emas=False)[
                "sigma"
