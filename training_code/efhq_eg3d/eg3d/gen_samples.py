...
@click.option(
    "--shapes",
    help="Export shapes as .mrc files viewable in ChimeraX",
    type=bool,
    required=False,
    metavar="BOOL",
    default=False,
...
@click.option(
    "--fov-deg",
    help="Field of View of camera in degrees",
    type=int,
    required=False,
    metavar="float",
    default=18.837,
...
def generate_images(
    ...
    cam2world_pose = LookAtPoseSampler.sample(
        3.14 / 2, 3.14 / 2, torch.tensor([0, 0, 0.2], device=device), radius=2.7, device=device
    )
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for angle_y, angle_p in [(0.4, angle_p), (0, angle_p), (-0.4, angle_p)]:
            ...
            cam2world_pose = LookAtPoseSampler.sample(
                np.pi / 2 + angle_y, np.pi / 2 + angle_p, cam_pivot, radius=cam_radius, device=device
            )
        ...
        if shapes:
            ...
            if shape_format == ".ply":
                ...
                with mrcfile.new_mmap(
                    os.path.join(outdir, f"seed{seed:04d}.mrc"), overwrite=True, shape=sigmas.shape, mrc_mode=2
                ) as mrc:
                    mrc.data[:] = sigmas
