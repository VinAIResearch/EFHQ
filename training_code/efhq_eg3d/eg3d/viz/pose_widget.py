    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 8):
                _clicked, self.lookat_point_choice = imgui.combo(
                    "", self.lookat_point_choice, self.lookat_point_labels
                )
            ...
            if lookat_point != "auto":
                ...
                with imgui_utils.item_width(viz.font_size * 16):
                    with imgui_utils.grayed_out(not changes_enabled):
                        _changed, self.lookat_point = imgui.input_float3(
                            "##lookat",
                            *self.lookat_point,
                            format="%.2f",
                            flags=(imgui.INPUT_TEXT_READ_ONLY if not changes_enabled else 0)
                        )
